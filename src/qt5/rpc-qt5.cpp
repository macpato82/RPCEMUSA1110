                    ...
                    int yl, int yh, int double_size, int host_xsize, int host_ysize)
{
	...
	//   Wrap the buffer in a QImage container, then take a deep copy so the
	//   message owns its pixel data. This is essential: the QImage is consumed
	//   later by the GUI thread via a queued signal, but 'buffer' points at the
	//   video thread's bitmap (thr.bitmap), which the machine thread may
	//   realloc() in resizedisplay() as soon as the video mutex is released
	//   (e.g. on a screen mode change). Without the copy the GUI thread would
	//   read freed/moved memory - a use-after-free that crashes on switching to
	//   large modes such as 1920x1080. The copy happens while the video thread
	//   still holds the mutex, so 'buffer' is guaranteed valid here.
	video_update.image = QImage((uchar *) buffer,
	    xsize, ysize, QImage::Format_RGB32).copy();
	video_update.yl = yl;
	video_update.yh = yh;
