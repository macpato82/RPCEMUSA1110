...
/* One dirty-buffer entry per 4KB page of VRAM. Sized for the maximum
   supported VRAM (16MB / 4KB = 4096 entries). */
#define DIRTYBUFFER_SIZE (16 * 1024 * 1024 / 4096)

/* Two dirty buffers, so one can be written to by the main thread
   while the display thread is reading the other */
static uint8_t dirtybuffer1[DIRTYBUFFER_SIZE];
static uint8_t dirtybuffer2[DIRTYBUFFER_SIZE];
...
vidcthread(void)
{
	...
	memset(thr.dirtybuffer, 0, DIRTYBUFFER_SIZE);
...
resetbuffer(void)
{
	memset(dirtybuffer, 0xff, DIRTYBUFFER_SIZE);
}
