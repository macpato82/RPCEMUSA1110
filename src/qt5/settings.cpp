...
config_load(Config * config)
{
	...
	/* Phoebe (RPC2) supports user-configurable RAM and VRAM like the Risc PC,
	   so its sizes are no longer forced here. The values loaded above (or the
	   defaults) are used and validated by the standard size checks. */
