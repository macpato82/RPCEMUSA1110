...
cp15_write(uint32_t opcode, uint32_t val)
{
	...
	case 8: /* TLB Operations (ARMv4) */
		if (cp15.cpu_model == CPUModel_SA110 || cp15.cpu_model == CPUModel_SA1110 ||
		    cp15.cpu_model == CPUModel_ARM810) {
			if (opc2 == 0) {
				/* TLB Flush */
				cp15_tlb_flush_all();
			} else {
...
cp15_read(uint32_t opcode)
{
	...
	case 0: /* ID */
		...
		/* StrongARM-compatible variant. A real SA-1110 reports implementer 0x69
		   (Intel), which the Risc PC RISC OS ROM does not recognise - it then
		   fails CPU setup and hangs at the banner. Even a same-part higher
		   revision is risky, so this model reports the exact SA-110 ID that the
		   ROM is known to accept, guaranteeing a clean boot. It is therefore
		   functionally identical to the StrongARM (SA110) model on this ROM. */
		case CPUModel_SA1110:    return 0x4401a102;
		}
		break;
	case 1: /* Control */
		return cp15.ctrl;
