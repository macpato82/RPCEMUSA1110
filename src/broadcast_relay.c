...
/* Sockets are made non-blocking via ioctlsocket(), so the recv flag is unused. */
#ifndef MSG_DONTWAIT
#define MSG_DONTWAIT 0
#endif

/* Windows lacks ifaddrs; broadcast address is found via GetAdaptersAddresses. */
static const char *
sock_strerror(void)
{
	static char buf[64];
...
get_broadcast_address(struct in_addr *bcast, struct in_addr *host)
{
    ...
    /* GetAdaptersAddresses may need a bigger buffer; retry once. */
    for (tries = 0; tries < 2; tries++) {
        ...
        if (ret == ERROR_BUFFER_OVERFLOW) {
            free(addrs);
            addrs = NULL;
            continue; /* buflen now holds the required size */
        }
        break;
    }

    if (addrs == NULL || ret != NO_ERROR) {
        rpclog("broadcast_relay: GetAdaptersAddresses() failed: %lu\n", (unsigned long) ret);
        free(addrs);
        return -1;
    }

    for (aa = addrs; aa != NULL && !found; aa = aa->Next) {
        ...
        for (ua = aa->FirstUnicastAddress; ua != NULL; ua = ua->Next) {
            ...
            mask = (prefix == 32) ? 0xffffffffu : ~((1u << (32 - prefix)) - 1);

            host->s_addr = sa->sin_addr.s_addr;
            bcast->s_addr = htonl(ip | ~mask);
            found = 1;

            rpclog("broadcast_relay: using host %s, ", inet_ntoa(*host));
            rpclog("broadcast %s\n", inet_ntoa(*bcast));
            break;
        }
    }
...
broadcast_relay_init(void)
{
...
#ifdef _WIN32
    {
        ...
        if (!wsa_started) {
            if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
                rpclog("broadcast_relay: WSAStartup() failed\n");
                return -1;
            }
            wsa_started = 1;
        }
    ...
    for (i = 0; i < NUM_ACCESS_SOCKETS; i++) {
        ...
        {
            u_long nb = 1;
            if (ioctlsocket(relay.sockets[i], FIONBIO, &nb) != 0) {
