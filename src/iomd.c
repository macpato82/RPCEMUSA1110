...
iomd_write(uint32_t addr, uint32_t val)
{
	...
	case IOMD_0x0CC_DMAEXT: /* DMA external control (IOMD/IOMD2) */
		if (iomd_type == IOMDType_IOMD || iomd_type == IOMDType_IOMD2) {
			/* IOMD2 retains the IOMD DMA external control register here;
			   keep the write path consistent with iomd_read() */
			iomd.dmaext = val;
			return;
		}
...
iomd_read(uint32_t addr)
{
	...
	case IOMD_0x0CC_DMAEXT: /* DMA external control     (IOMD/IOMD2) */
		if (iomd_type != IOMDType_ARM7500 && iomd_type != IOMDType_ARM7500FE) {
			/* IOMD and IOMD2 expose DMA external control here */
			return iomd.dmaext;
		}
...
iomd_reset(IOMDType type)
{
	...
	if (iomd_type == IOMDType_ARM7500 || iomd_type == IOMDType_ARM7500FE) {
		...
		/* IOMD and IOMD2 (Risc PC / RPC2) only */
		iomd.refcr = 0;      /* DRAM refresh */
		iomd.dmaext = 0;     /* DMA external control */
	}
